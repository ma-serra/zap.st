...
const replaceEncryptionMessageAuthor = (messages: IndexedMessage[]) =>
  messages.map((message, i) => {
    ...
    if (message.message.includes('end-to-end') || 
        message.message.includes('criptografia') ||
        message.message.includes('protegidas') ||
        message.message.toLowerCase().includes('encryption')) {
      return { ...message, author: null, message: 'App criado para organizar o arquivo gerado pelo WhatsApp ao exportar conversa com mídia.' };
    }
    
    // Substituir mensagens de criação de grupo
    if (message.message.includes('created group') || 
        message.message.includes('criou o grupo') ||
        message.message.includes('criou este grupo') ||
        message.message.toLowerCase().includes('created') && message.message.toLowerCase().includes('group')) {
...
async function generateCompleteHTML(messages: IndexedMessage[], extractedFile: ExtractedFile, activeUser: string): Promise<string> {
  ...
  const captureRealDOM = (): string => {
    try {
      ...
      const messageContainer = document.querySelector('[data-testid="message-list"], .message-list, ul') as HTMLElement;
      ...
      const captureComputedStyles = (element: Element): string => {
        ...
        const importantProps = [
          ...
          'text-align', 'vertical-align', 'white-space', 'word-wrap', 'overflow-wrap',
  ...
  if (!realDOMHTML) {
    ...
    const authorColors = [
      '#1f7aec', '#fe7c7f', '#6bcbef', '#fc644b', '#35cd96', 
      '#e542a3', '#91ab01', '#ba33dc', '#ffa97a', '#029d00', '#dfb610'
    ];
    ...
    for (let i = 0; i < messages.length; i++) {
      ...
      const sameAuthorAsPrevious = message.author === previousAuthor && !isSystem;
      const authorColor = message.author ? authorColors[message.author.length % authorColors.length] : '#1f7aec';
      ...
      if (message.attachment) {
        ...
        messageContent = message.message
          .replace(/https?:\/\/[^\s]+/g, '<a href="$&" target="_blank" rel="noopener noreferrer" style="color: #68bbe4; text-decoration: underline;">$&</a>')
          .replace(/\n/g, '<br>');
      }
      ...
      messagesHTML += `
        <li style="${itemStyle}">
          <div style="${bubbleStyle}">
            ...
            ${!isSystem ? `<time datetime="${message.date.toISOString()}" style="flex: 0 0 auto; align-self: flex-end; margin-left: 1rem; white-space: nowrap; font-size: 75%; opacity: 0.6; ${window.innerWidth <= 699 ? 'margin-top: 0.25rem; margin-left: 0;' : ''}">${new Intl.DateTimeFormat('default', {
              year: 'numeric',
              month: 'short',
              day: 'numeric',
              hour: 'numeric',
